...
- **Fractional Calculus**: Native support for non-integer order differentiation (Caputo definition) and automated FDE solving with initial conditions.
- **Robust Exception Hierarchy**: Introduced `NablaSolveError` and `LaplaceTransformError`.

## [1.0.2] - 2026-04-19

### Security & Robustness
- **DoS Protection**: Set `MAX_LATEX_LENGTH = 10000` in the `parse()` function to restrict memory consumption.
- **Evaluation Stability**: Deterministic dictionary key sorting (`sorted(subs.keys())`) added in `_vectorized_evaluate` for `lambdify` cache stability.
